...
if lsof -i :$PORT > /dev/null 2>&1; then
    ...
    if ! lsof -i :$PORT > /dev/null 2>&1; then
        echo -e "\033[0;31mErreur: Le serveur n'a pas démarré. Consultez server/server.log${NC}"
        exit 1
    fi
    PID=$(lsof -t -i :$PORT)
fi
...
echo -e "${BLUE}║${NC}  ${GREEN}PID:${NC}    ${WHITE}$PID${NC}                                                  ${BLUE}║${NC}"
echo -e "${BLUE}╠═══════════════════════════════════════════════════════════════╣${NC}"
...
echo -e "${BLUE}║${NC}  Logs:     ${WHITE}tail -f server/server.log${NC}                            ${BLUE}║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════════════════╝${NC}"
echo ""
